...
namespace WARETRACK
{
    public partial class frm_satın_alma_irsaliyeleri : Form
    {
        public frm_satın_alma_irsaliyeleri()
        {
            InitializeComponent();
        }

        private void frm_satın_alma_irsaliyeleri_Load(object sender, EventArgs e)
        {
            // TODO: This line of code loads data into the 'wARETRACKDataSet1.TALLYIN' table. You can move, or remove it, as needed.
            this.tALLYINTableAdapter.Fill(this.wARETRACKDataSet1.TALLYIN);
        ...
        private void bunifuThinButton21_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (dr.Read())
                {
                    //alternatif çözüm
                    dr.Close();
                    SqlCommand komut2 = new SqlCommand("select * from TALLYIN where FICHENO=@p1", baglan);
                    ...
                    else
                    {
                        ...
                        SqlCommand komut = new SqlCommand("insert into TALLYIN (FICHENO, DATE_, DOCNO, PRCODE, COUNT_, PRICE) values (@p1,@p2,@p3,@p4,@p5,@p6)", baglan);
                        ...
                        baglan.Close();
                        MessageBox.Show("Satın alma irsaliyesi kaydedildi");
                        this.tALLYINTableAdapter.Fill(this.wARETRACKDataSet1.TALLYIN);
                        txt_urun_kodu.Text = "";
                        txt_adet.Text = "";
        ...
        private void btn_kaydet_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SqlConnection baglan = new SqlConnection(yol);
                baglan.Open();
                SqlCommand komut = new SqlCommand("Update TALLYIN Set DATE_=@p2, DOCNO=@p3, COUNT_=@p4, PRICE=@p5, PRCODE=@p6 where FICHENO=@p1", baglan);
                komut.Parameters.AddWithValue("@p1", txt_irsaliye_no.Text.Trim());
                komut.Parameters.AddWithValue("@p2", Convert.ToDateTime(txt_tarih.Text.Trim()));
                komut.Parameters.AddWithValue("@p3", txt_belge_no.Text.Trim());
                int adet = Convert.ToInt32(txt_adet.Text.Trim());
        ...
        private void btn_sil_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SqlConnection baglan = new SqlConnection(yol);
                baglan.Open();
                SqlCommand komut = new SqlCommand("Delete From TALLYIN Where FICHENO=@p1", baglan);
                komut.Parameters.AddWithValue("@p1", txt_irsaliye_no.Text.Trim());
                komut.ExecuteNonQuery();
                baglan.Close();
        ...
        private void dtg_satın_alma_irsaliyeleri_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int secim = dtg_satın_alma_irsaliyeleri.SelectedCells[0].RowIndex;
            txt_irsaliye_no.Text = dtg_satın_alma_irsaliyeleri.Rows[secim].Cells[0].Value.ToString().Trim();
            txt_tarih.Text = Convert.ToDateTime(dtg_satın_alma_irsaliyeleri.Rows[secim].Cells[1].Value.ToString().Trim()).ToShortDateString();
            txt_belge_no.Text = dtg_satın_alma_irsaliyeleri.Rows[secim].Cells[2].Value.ToString().Trim();
            txt_urun_kodu.Text = dtg_satın_alma_irsaliyeleri.Rows[secim].Cells[3].Value.ToString().Trim();
            txt_adet.Text = dtg_satın_alma_irsaliyeleri.Rows[secim].Cells[4].Value.ToString().Trim();
            txt_fiyat.Text = dtg_satın_alma_irsaliyeleri.Rows[secim].Cells[5].Value.ToString().Trim();
        }
    }
}
